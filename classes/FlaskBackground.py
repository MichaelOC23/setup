...
class prospect_data_pipeline:
    ...
    async def async_process_prospect(self, propsect_data):
        ...
        def search_you_rag_content(query):
            ydc_list = []
        ...
        for title in self.titles_list:
            ...
            you_results = search_you_rag_content(search_query)
...
def startaudiorec():
    ...
    mic_thread = threading.Thread(target=capture_audio, args=(1, 1, audio_queue))
...
#             google_results = await self.async_search_google(search_query)
            
#             add_you_results = await self.async_search_you_rag_content(search_query)
